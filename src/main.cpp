...
int main(int argc, char **argv)
{
  ...
  while (PobierzNastpnePytanie(&BazaT,&WyrZ_PytanieTestowe)) {
    ...
    if(b_zapisu == 3){
      cin.clear();
      cin.ignore(1000,'\n');
      cout << ":( Trzeci błąd zapisu liczby zespolonej. Przejscie do kolejnego pytania." << endl;
      ZlaOdp(Pkt);
    }
    else if(OdpWlasc != OdpUzyt){
      cout << ":( Błąd. Prawidłowym wynikiem jest: "<< OdpWlasc << endl;
      cout << "   Wpisano: "<< OdpUzyt << endl;
      ZlaOdp(Pkt);
    }
    else{
